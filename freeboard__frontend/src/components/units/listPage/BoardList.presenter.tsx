...
<<<<<<< Updated upstream
          <Search type="text" placeholder="제목을 검색해주세요."></Search>
=======
          <Search
            type="text"
            name="title"
        ...
        <div>
          {data?.fetchBoards.slice(0, 10).map((data, index) => (
            <ContentWrapper key={data._id}>
              <ContentNum>{(currentPage - 1) * 10 + index + 1}</ContentNum>
              <ContentTitle onClick={handleListClick} id={data._id}>
                {data.title}
              </ContentTitle>
              <ContentWriter>{data.writer}</ContentWriter>
              <ContentData>{data.createdAt.slice(0, 10)}</ContentData>
            </ContentWrapper>
          ))}
        </div>
      </LineWrapper>
...
>>>>>>> Stashed changes
        ...
        <EnrollmentWrapper>
          ...
          <Enrollment onClick={handleCreatePage}>게시물 등록하기</Enrollment>
        </EnrollmentWrapper>
      </EndWrapper>
    </Wrapper>
  );
